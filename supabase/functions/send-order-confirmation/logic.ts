// Бизнес-логика для Edge Function (извлечена для тестирования)
...
export function generateCustomerEmail(data: OrderData): string {
...
<body style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; line-height: 1.6; color: #1f2937; margin: 0; padding: 0; background-color: #f3f4f6;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: white; padding: 32px; border-radius: 0 0 16px 16px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
      ...
      <p style="margin: 0; color: #6b7280; font-size: 14px;">Отследить статус заказа:<br><a href="${BASE_URL}/track?order=${data.orderNumber}" style="color: #1e40af;">Перейти к отслеживанию</a></p>
      <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 32px 0;">
...
export function generateAdminEmail(data: OrderData): string {
...
<body style="font-family: sans-serif; line-height: 1.6; color: #1f2937; margin: 0; padding: 20px; background-color: #f3f4f6;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; padding: 24px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
    ...
    <p style="margin: 24px 0 0; text-align: center;">
      <a href="${BASE_URL}/admin#/orders" style="display: inline-block; background: #1e40af; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold;">Открыть в админке</a>
    </p>
  </div>
</body></html>`
}

export function generateTelegramMessage(data: OrderData): string {
  const accessoriesList = data.accessories?.length
    ? data.accessories.map(acc => `• ${acc.name}`).join('\n')
    : 'Нет'

  // Ссылка на админку с поиском по номеру заказа
  const adminUrl = `${BASE_URL}/admin#/orders?filter=${encodeURIComponent(JSON.stringify({ q: data.orderNumber }))}`
...
export function parseChatIds(chatIdsStr: string | undefined): string[] {
  if (!chatIdsStr) return []
  return chatIdsStr.split(',').map(id => id.trim()).filter(id => id)
}

export function validateOrderData(data: unknown): data is OrderData {
...
export async function sendEmail(
  ...
  const response = await deps.fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: deps.fromEmail, to: params.to, subject: params.subject, html: params.html }),
  })
...
export async function sendTelegramMessage(
  ...
  if (!response.ok) {
    const errorData = await response.text()
    return { success: false, error: `Failed to send Telegram message to ${chatId}: ${errorData}` }
  }
  return { success: true }
}
