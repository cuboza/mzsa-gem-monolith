      ...
      if (path === '/trailers' && req.query.q) {
        ...
        if (q.includes('лодк') || q.includes('катер') || q.includes('boat') || q.includes('water')) {
          conditions.push({ category: 'water' });
           ...
           // Assuming we might have a category or compatibility check
           // For now, let's check compatibility in JSON or category
           conditions.push({ 
             [Op.or]: [
               { category: 'moto' },
               // SQLite JSON querying can be tricky, simple text match for now
...
// --- Routes ---
createCrud(app, '/trailers', Trailer);
