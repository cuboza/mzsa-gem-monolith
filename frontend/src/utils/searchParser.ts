...
export function parseSearchQuery(query: string): ParsedSearch {
  ...
  if (!result.volume) {
    // Варианты: 3.5м, 4 м, 350см, 3500мм, 4000 мм, 3,5 метра
    // ВАЖНО: Числа БЕЗ единиц измерения парсим ТОЛЬКО при наличии категории техники!
    // Иначе "Volzhanka 46 Fish" превратится в 46 метров
    ...
    // Числа без единиц измерения парсим ТОЛЬКО если есть категория техники
    // Это предотвращает интерпретацию "46" из "Volzhanka 46 Fish" как размера
    if (!result.length && result.category) {
      const numberOnlyPattern = /(?:^|\s)(\d+(?:[.,]\d+)?)(?:\s|$)/;
      const numberMatch = lower.match(numberOnlyPattern);
      if (numberMatch) {
